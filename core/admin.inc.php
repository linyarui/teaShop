...
function getAllAdmin(){
	...
	$sql="select id,username,email from tea_admin ";
	$rows=fetchAll($sql);
	return $rows;
}
function getAdminByPage($page,$pageSize=2){
	$sql="select * from tea_admin";
	global $totalRows;
	...
	$sql="select id,username,email from tea_admin limit {$offset},{$pageSize}";
	$rows=fetchAll($sql);
	return $rows;
}
 ...
 */
function delUser($id){
	$sql="select face from tea_user where id=".$id;
	$row=fetchOne($sql);
	$face=$row['face'];
	if(file_exists("../uploads/".$face)){
		unlink("../uploads/".$face);
	}
	if(delete("tea_user","id={$id}")){
